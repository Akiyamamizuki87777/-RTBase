...
public:
	...
	Colour evaluate(const ShadingData& shadingData, const Vec3& wi)
	{
		...
		// Lambert反射率 = albedo * cos(theta) / π
		float cosTheta = std::max(0.0f, wiLocal.z);
...
public:
	...
	Vec3 sample(const ShadingData& shadingData, Sampler* sampler, Colour& reflectedColour, float& pdf)
	{
		...
		// 计算 θ 和 φ
		float theta = acos(sqrt(xi1));  // θ = cos^(-1)(sqrt(ξ1))
		float phi = 2.0f * M_PI * xi2;  // φ = 2πξ2
		...
		// 计算 PDF
		pdf = cos(theta) / M_PI;  // p(ω) = cos(θ) / π
		//====================================================
		reflectedColour = albedo->sample(shadingData.tu, shadingData.tv) / M_PI;
		wi = shadingData.frame.toWorld(wi);
		return wi;
	...
	float PDF(const ShadingData& shadingData, const Vec3& wi)
	{
		...
		Vec3 wiLocal = shadingData.frame.toLocal(wi);
		//return SamplingDistributions::cosineHemispherePDF(wiLocal);
		// 计算cos(θ)，这里假设wiLocal.z表示cos(θ)
		float cosTheta = wiLocal.z;

		// 计算PDF，按照PPT中的公式 p(ω) = cos(θ) / π
